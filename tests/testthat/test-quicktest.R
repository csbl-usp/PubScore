...
test_that("test literature  score works when removing ambiguous terms", {
  ...
  terms_of_interest <- c("B cells", "macrophages", "NK cells")
  pub <- pubscore(terms_of_interest = terms_of_interest, genes = selected_genes )
  pub <- test_score(pub, total_genes =  total_genes, remove_ambiguous = TRUE, nsim = 1e4)
  p1 <- pub@p_value
  expect_that(pub@p_value, is_less_than(0.01))
  pub <- test_score(pub, total_genes =  total_genes, remove_ambiguous = TRUE, ambiguous_terms = c('PC', "ACHE", "IMPACT"), nsim = 1e4)
  expect_that(pub@p_value, is_less_than(0.2))
  expect_that(pub@p_value, is_more_than(0.05))
})


#### test_score for gene2pubmed yet not implemented ####
...
#'   p1 <- pub@p_value
#'   expect_that(pub@p_value, is_less_than(0.01))
#'   pub <- test_score(pub, total_genes =  total_genes, remove_ambiguous = TRUE, ambiguous_terms = c("SHE"), nsim = 1e4)
#'   expect_that(pub@p_value, is_less_than(0.01))
#' })

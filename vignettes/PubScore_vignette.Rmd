...
#' pub <- test_score(pub, total_genes = total_genes)
#' all_counts <- pub@all_counts
#' save(all_counts, file= '../data/all_counts.RData')
data("all_counts")
pub@all_counts <- all_counts
```
...
simulation_of_literature_null <- pub@all_counts
...
```{r}
head(pub@all_counts$Genes[order(pub@all_counts$count, decreasing = TRUE)], 10)
```
...
```{r}
retested_literature_object <- test_score(literature_object, 

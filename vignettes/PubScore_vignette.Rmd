...
remotes::install_github('lubianat/PubScore')
library(PubScore)
# list of genes :
selected_genes <- c('CD79A', 'CD18', 'NKG7', 'CST3', 'AIF1')

# These genes were selected from a panel containing the following genes
total_genes <- c('CD79A', 'CD18', 'NKG7', 'CST3', 'AIF1', 'FOXA1', 'PPT2', 'ZFP36L1','AFF4', 'ANTXR2', "HDAC8", "VKORC1" )

terms_of_interest <- c("B cells", "macrophages", "NK cells")
```


First, let's observe the literature object produced by the function. 

```{r}
literature_object <- PubScore::get_literature_score(selected_genes, terms_of_interest = terms_of_interest, is.list = TRUE)

print('The score for the list is:')
print(literature_object$list_literature_score)
```

This score is the average number of articles for each gene x term combination of the list on PubMed. In the object we also have the date and a pre-set max_score. 

The number does not mean much. Higher values mean that the genes are more associated to the terms. But, what is the score I expect by chance? Let's run a permutation test to estimate this p-value.

For this, we need the list of all genes that could have been selected. This was assigned above to the variable total_genes


```{r}
literature_object_tested <- test_literature_score(literature_object,
total_genes,                                                          remove_ambiguous = TRUE)
...
```{r}
p <- PubScore::plot_literature_score(literature_object_tested$counts, return_ggplot = TRUE)
library(plotly)
ggplotly(p)
```
#Advanced Usage
...
literature_object <- PubScore::get_literature_score(gene = fcbf_genes,is.list = TRUE,terms_of_interest = terms_of_interest)
library(ggplot2)

p <- plot_literature_score(plot_counts = literature_object$counts, return_ggplot = TRUE)
library(plotly)
ggplotly(p)
```
...
```{r}
literature_object$all_gene_combinations$Gene[order(literature_object$all_gene_combinations$count, decreasing = TRUE)][1:30]
```
...
retested_literature_object <- retest_literature_score(literature_object, remove_ambiguous = TRUE, nsim = 10000)
```

The pvalue after removing the noise is around 0.043. It will oscilate slightly everytime, as there is a component of randomness in the simulation.
...
In this case, the p-value is even shorter than the old-fashioned 0.05, so I can say confidently that the list returned by FCBF is more related to the "Dengue" term" than expected by chance. 

To finish this vignett, if you want to select your own list of ambiguously named genes, you can do so in the retest function. 

```{r}
retested_literature_object <- retest_literature_score(literature_object, 
                                                      new_ambigous_term_list = 
                                                      c("PC", "JUN", "IMPACT"), nsim = 10000)

```

Of course, this parameter is likely to afect the final result. 
